...
class ACPClient:
    ...
    def _route_message(self, message: Dict[str, Any]) -> None:
        ...
        if has_id and not has_method:
            ...
            if has_id:
                ...
                logger.info(f"ACPClient: Received request from server: {method} id={msg_id}")
                ...
                logger.debug(f"ACPClient: Routing notification method={method} to {len(self.notification_handlers)} handlers")
                for handler in self.notification_handlers:
                    try:
                        handler(message)
                    except Exception as e:
    ...
    def _send_request(self, method: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            response = response_queue.get(timeout=120)  # Increased timeout for long-running prompts
    ...
    def send_prompt(self, session_id: str, content: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.process and self.process.poll() is not None:
                logger.error(f"kiro-cli process has exited with code: {self.process.poll()}")
            raise
    ...
    def respond_to_permission(self, request_id, session_id: str, tool_call_id: str, option_id: str) -> None:
        ...
        logger.info(f"Responding to permission request id={request_id}: optionId={option_id}")
